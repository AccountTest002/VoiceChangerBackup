...
public class PurchaseManager {
    ...
    public boolean isPurchased() {
        if (!AdmobManager.getInstance().isHasAds()) {
            return true;
        }
        for (int i = 0; i < purchaseList.size(); i++) {
    ...
    public String getPriceSubscription(String productId) {
        if (productDetailsList != null) {
            for (int i = 0; i < productDetailsList.size(); i++) {
                if (productDetailsList.get(i).getProductId().equals(productId)) {
                    {
//                        get price in app purchase
//                        ProductDetails.OneTimePurchaseOfferDetails detail = productDetailsList.get(i).getOneTimePurchaseOfferDetails();
//                        if (detail != null) {
//                            return detail.getFormattedPrice();
//                        }
...
//                        get price sub
                        String details = productDetailsList.get(i).getSubscriptionOfferDetails().get(i).getPricingPhases().getPricingPhaseList().get(i).getFormattedPrice();
                        return details;
                    }
                }
            }
